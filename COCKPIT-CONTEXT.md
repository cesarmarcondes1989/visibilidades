...
| `initPeopleGraph()` | Cria `peopleGraph` (instância `ForceGraph` dedicada, mostra todos os tipos de `getEffectiveGraphNodes()` filtrados por `isGraphVisible` por padrão; `peopleGraphFilter` vazio = todos os tipos, ou só os selecionados nos chips) |
| `refreshPeopleGraph()` | Destrói e recria `peopleGraph` (mirror de `_refreshGraph()`) |
| `initGraph()` | Cria instância `ForceGraph` com nós filtrados por `graphFilter` (Set multi-seleção; vazio = todos os tipos) e por `isGraphVisible` (esconde projetos não-Active) |
| `getEffectiveGraphNodes()` | Retorna nós não-derivados do `gNodes` + nós derivados de `projects` (tipo `projeto`, **todos** os status) + nós derivados de `people` (tipo `pessoa`). É o resolver completo — usado para resolver label/tipo de qualquer nó, incluindo conexões antigas para projetos hoje não-Active |
| `isGraphVisible(n)` | `true` para qualquer nó que não seja `projeto`; para nós de projeto, só `true` se `projects.find(p=>"pj-"+p.id===n.id).status==="Active"`. Filtra o canvas do Graph/People e as listas de destino de `openAddEdgePanel`/`openAddEdgePanelGlobal` — existe para não poluir o grafo com 150+ projetos quando só uma fração está ativa. **Não** é aplicado em `getEffectiveGraphNodes()` (resolver completo) nem nas listas de conexões já existentes (`openNodePanel`, `renderPersonConnArea`), que continuam mostrando o nome correto mesmo para projetos não-Active |
...
### Graph — Apenas projetos Active aparecem no canvas e nos seletores de conexão
Com 150+ projetos cadastrados, desenhar todos como nó no Graph/People tornaria o grafo ilegível. `isGraphVisible(n)` filtra nós de projeto para mostrar só os com `status==="Active"` (outros tipos de nó — pilar/produto/domínio/pessoa — não são afetados). Esse filtro é aplicado em 4 pontos: `initGraph()` e `initPeopleGraph()` (canvas), e nas opções de destino de `openAddEdgePanel()`/`openAddEdgePanelGlobal()` (não dá para criar uma nova conexão para um projeto não-Active). **Conexões já existentes para projetos não-Active continuam intactas e visíveis** nas listas que usam o resolver completo (`getEffectiveGraphNodes()`, sem o filtro) — `openNodePanel()` (lista de Conexões do nó) e `renderPersonConnArea()` (lista de Projetos conectados da pessoa) seguem mostrando o nome correto do projeto antigo, mesmo que ele não apareça mais desenhado no canvas. Ou seja: o projeto "some" do desenho do grafo quando perde o status Active, mas o histórico de conexões não é apagado nem corrompido — só fica oculto até o projeto voltar a Active (ou até alguém remover a conexão manualmente).
